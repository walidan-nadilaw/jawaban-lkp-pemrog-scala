...
object lihat2baris {
    def main (args: Array[String]) : Unit ={
        ...
        val names = List.fill(n)(readLine().split(",").map(_.trim).toList)

        def namesTOmovies ( nameLIST: List[List[String]]) : List[List[String]] = {
            val max_length = nameLIST.map(_.size).max

            val fill_missing = nameLIST.map( row => row.map( Some(_)) ++ List.fill(max_length - row.length)(None) )
            val transpose = fill_missing.transpose
            val movieLIST = transpose.map(_.flatten)
        ...
        def cek2movies ( a: Int, b:Int, movies:List[List[String]] ) : Unit = {
            println(movies(a))
            println(movies(b))
            val intersection = movies(a).intersect(movies(b))
        ...
        val movies = namesTOmovies(names)
